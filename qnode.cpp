...
void QNode::imageCallback(const sensor_msgs::ImageConstPtr &msg_left,
  ...
  if(m_overlaying)
  {
      ...
      rook_points[0][0] = cv::Point( w/4.0, 7*h/8.0 );
      rook_points[0][1] = cv::Point( 3*w/4.0, 7*h/8.0 );
      rook_points[0][2] = cv::Point( 3*w/4.0, 13*h/16.0 );
      rook_points[0][3] = cv::Point( 11*w/16.0, 13*h/16.0 );
      rook_points[0][4] = cv::Point( 19*w/32.0, 3*h/8.0 );
      rook_points[0][5] = cv::Point( 3*w/4.0, 3*h/8.0 );
      rook_points[0][6] = cv::Point( 3*w/4.0, h/8.0 );
      rook_points[0][9] = cv::Point( 26*w/40.0, h/8.0 );
      rook_points[0][8] = cv::Point( 26*w/40.0, h/4.0 );
      rook_points[0][7] = cv::Point( 22*w/40.0, h/4.0 );
      rook_points[0][10] = cv::Point( 22*w/40.0, h/8.0 );
      rook_points[0][11] = cv::Point( 18*w/40.0, h/8.0 );
      rook_points[0][12] = cv::Point( 18*w/40.0, h/4.0 );
      rook_points[0][13] = cv::Point( 14*w/40.0, h/4.0 );
      rook_points[0][14] = cv::Point( 14*w/40.0, h/8.0 );
      rook_points[0][15] = cv::Point( w/4.0, h/8.0 );
      rook_points[0][16] = cv::Point( w/4.0, 3*h/8.0 );
      rook_points[0][17] = cv::Point( 13*w/32.0, 3*h/8.0 );
      rook_points[0][18] = cv::Point( 5*w/16.0, 13*h/16.0 );
      rook_points[0][19] = cv::Point( w/4.0, 13*h/16.0) ;

      const cv::Point* ppt[1] = { rook_points[0] };
      int npt[] = { 20 };

        cv::fillPoly( image_left,
                  ppt,
                  npt,
                  1,
                  cv::Scalar( 255, 0, 0 ),
